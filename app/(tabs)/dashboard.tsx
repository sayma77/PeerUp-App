...
// requests where requester==uid). These stay mocked until we build

// Requests/Reviews — "My Skills" and "Project Requests" are both wired to
...
export default function Dashboard() {
  ...
  // ── Team-Fit Suggestions: per-request AI fit score, keyed by

  // `${projectId}-${userId}`. Populated on-demand when the creator taps

  // "Analyze Fit" on a join request — see handleAnalyzeFit below.
  ...
  // Only the projects the current user created show up under

  // "Project Requests" — that's the person who should see/manage them.
  ...
  async function handleAnalyzeFit(project: Project, request: JoinRequest) {
    ...
    try {
      ...
      // The requester's skills live in Firestore (same "skills" collection

      // used by the Skills tab), keyed by mentorId === their uid.
  ...
  async function handleDeclineProjectRequest(projectId: string, userId: string) {
